// En esta hoja se realizan pruebas del js para evitar hacer ramas.
//Eneste momento actualizando la estructura del proyecto.
let datoUsuario
...
function preguntarUsuario(opcionUsuario, arrayPregunta, arrayOpcion) {

    while (preguntasTotales > 0) {

        if (opcionUsuario != arrayPregunta[arrayOpcion[0]]) {
            contadorDePuntos()
...
function preguntasTotalesF() {
    for (const pregunta of askUCM) {
        if (pregunta) {
            preguntasTotales++
        }
    }
...
function preguntasUCM() {
    askUCM = [{
            ...
            opciones: ["Chitauri", "Klyntar", "Kronans"],
            preguntasTotalesF()
        },

        {
            pregunta: "¿Por quien consigue Clint la gema del Alma?",
            opciones: ["Viuda Negra", "Bruja Escarlata", "Gamora", "Carol Danvers"],
            preguntasTotalesF()
        }

    ]
    preguntarUsuario(datoUsuario)
    jugarDeNuevo()
